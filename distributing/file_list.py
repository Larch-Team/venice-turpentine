...
for socket in os.listdir('app/appdata/plugins'):
    ...
    for plugin in os.listdir(f'app/appdata/plugins/{socket}'):
        if '__pycache__' in plugin:
            ...
            plugins[socket][plugin] = [os.path.join(root.removeprefix('app/appdata/'), name) for root, dirs, files in os.walk(f'app/appdata/plugins/{socket}/{plugin}') for name in files if '__pycache__' not in root]

...
class Session(object):
    ...
    def save_proof(self, filename: str):
        ...
        if any(i.auto for i in self.proof.metadata['usedrules']):
            raise EngineError('Cannot save autogenerated proofs')
    ...
    def load_proof(self, filename: str):
        ...
        with open(f'saved_proofs/{filename}') as save_file:
            ...
            for i,j in state['setup']['chosen_plugins'].items():
                self.plug_switch(i, j)
            self.change_accessibility(state['setup']['accessibility'])
